...
Codec is a [token-native binary transport](https://codecai.net) for AI APIs. It carries token IDs as 4-byte frames in place of 50 to 100 bytes of UTF-8/JSON per token. The Codec wire format is going through upstream as two PRs against [sgl-project/sglang](https://github.com/sgl-project/sglang):
...
Any HF repo id (or local path inside the container). The supervisor downloads and boots on first start, then caches in the volume.
...
The directory is read-only inside the container. The supervisor cannot mutate your weights.
...
Pass `CODEC_BACKEND_ARGS` to tune sglang per-model (`--tp 2 --quantization fp8 --mem-fraction-static 0.9`, etc.). For per-load tuning, include `extra_args` in the `/admin/load` body. That overrides the supervisor's default for that one model.
...
- **Logits-space enforcement** (`codec_supervisor/safety_logits.py`). Banned-token-ID masking compatible with the vLLM
...
- **Admin REST surface** (`codec_supervisor/admin_safety.py`). Mounted at `/admin/policies/*`; see the admin-API table above.
